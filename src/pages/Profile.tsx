...
export default function ProfilePage() {
  ...
  const doLinkPassword = async () => {
    ...
    if (hasPasswordProvider) {
      pushToast({ type: "info", title: "Email/senha já vinculado a esta conta." });
      return;
    }
    if (!user.email) {
    ...
    if (linkPassword !== linkPasswordConfirm) {
      pushToast({ type: "error", title: "As senhas não coincidem." });
      return;
    }
      ...
      <Card className="py-12">
        ...
        {hasGoogleProvider ? (
          <div className="mt-10">
            ...
            {hasPasswordProvider ? (
              <div className="text-sm text-emerald-600 font-bold">Email/senha já vinculado a esta conta.</div>
            ) : (
